  ...
  const amountInUSDC = parseUnits(String(stakeAmount), 6);
   ...
   useEffect(() => {
     if (startTime && endTime && new Date(endTime) <= new Date(startTime)) {
       // auto-correct or clear the endTime if it's before startTime
       setValue("endTime", startTime); // or set to startTime
...
if(isConnected){
  ...
  console.log("The receipt response is : "  , receipt);
 if(receipt.status=="success"){
  ...
  if(response.status==200){
    console.log("The Response has been submitted " , session?.user._id);
  }else {
  ...
  alert("Please connect your wallet first");
      ...
      console.log("The Error in make pledge is :  " , error);
  ...
  return (
    ...
    >
      <h2 className="text-center gradient-text font-bold text-4xl sm:text-5xl mb-8">
        Let&apos;s make a pledge
      </h2>
      <form
        className="w-full max-w-md glass p-10 flex flex-col gap-8"
      ...
      >
        ...
        {session?.user && stakeAmount ? (
          <button
            type="submit"
            className="mt-2 bg-gradient-to-r from-blue-600 to-purple-500 text-white px-8 py-3 rounded-full font-semibold shadow-md hover:scale-105 hover:shadow-lg transition text-lg backdrop-blur-sm"
          >
            Pledge It
          </button>
        ) : (
